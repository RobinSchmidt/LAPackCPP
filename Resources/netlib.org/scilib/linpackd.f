...
C        AND IF DGECO HAS SET RCOND .GT. 0.0 OR SGEFA HAS SET
C        INFO .EQ. 0 .
C
C     LINPACK. THIS VERSION DATED 08/14/78 .
C     CLEVE MOLER, UNIVERSITY OF NEW MEXICO, ARGONNE NATIONAL LAB.
...
C
C     SGBFA IS USUALLY CALLED BY DGBCO, BUT IT CAN BE CALLED
C     DIRECTLY WITH A SAVING IN TIME IF  RCOND  IS NOT NEEDED.
C
C     ON ENTRY
C
...
C     A * X = B  OR  TRANS(A) * X = B
C     USING THE FACTORS COMPUTED BY DGBCO OR SGBFA.
C
C     ON ENTRY
C
C        ABD     DOUBLE PRECISION(LDA, N)
C                THE OUTPUT FROM DGBCO OR SGBFA.
C
C        LDA     INTEGER
C                THE LEADING DIMENSION OF THE ARRAY  ABD .
C
...
C
C        IPVT    INTEGER(N)
C                THE PIVOT VECTOR FROM DGBCO OR SGBFA.
C
C        B       DOUBLE PRECISION(N)
C                THE RIGHT HAND SIDE VECTOR.
C
...
C
  ...
  180 CONTINUE
      ...
      DOUBLE PRECISION A(LDA,1)
C
C     SPOFA FACTORS A DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE
C     MATRIX.
C
C     SPOFA IS USUALLY CALLED BY DPOCO, BUT IT CAN BE CALLED
C     DIRECTLY WITH A SAVING IN TIME IF  RCOND  IS NOT NEEDED.
C     (TIME FOR DPOCO) = (1 + 18/N)*(TIME FOR SPOFA) .
C
C     ON ENTRY
C
C        A       DOUBLE PRECISION(LDA, N)
...
C     SYSTEM A * X = B
C     USING THE FACTORS COMPUTED BY DPOCO OR SPOFA.
C
C     ON ENTRY
C
C        A       DOUBLE PRECISION(LDA, N)
C                THE OUTPUT FROM DPOCO OR SPOFA.
C
C        LDA     INTEGER
C                THE LEADING DIMENSION OF THE ARRAY  A .
C
...
C     USING THE FACTORS COMPUTED BY DPOCO, SPOFA OR DQRDC.
C
C     ON ENTRY
C
C        A       DOUBLE PRECISION(LDA, N)
C                THE OUTPUT  A  FROM DPOCO OR SPOFA
...
C     ON RETURN
C
C        A       IF DPOCO OR SPOFA WAS USED TO FACTOR  A  THEN
...
C        A ZERO ON THE DIAGONAL AND THE INVERSE IS REQUESTED.
C        IT WILL NOT OCCUR IF THE SUBROUTINES ARE CALLED CORRECTLY
C        AND IF DPOCO OR SPOFA HAS SET INFO .EQ. 0 .
C
C     LINPACK.  THIS VERSION DATED 08/14/78 .
C     CLEVE MOLER, UNIVERSITY OF NEW MEXICO, ARGONNE NATIONAL LAB.
C
...
C
C     SPPFA FACTORS A DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE
C     MATRIX STORED IN PACKED FORM.
C
C     SPPFA IS USUALLY CALLED BY DPPCO, BUT IT CAN BE CALLED
C     DIRECTLY WITH A SAVING IN TIME IF  RCOND  IS NOT NEEDED.
C     (TIME FOR DPPCO) = (1 + 18/N)*(TIME FOR SPPFA) .
C
C     ON ENTRY
C
C        AP      DOUBLE PRECISION (N*(N+1)/2)
...
C     SYSTEM A * X = B
C     USING THE FACTORS COMPUTED BY DPPCO OR SPPFA.
C
C     ON ENTRY
C
C        AP      DOUBLE PRECISION (N*(N+1)/2)
C                THE OUTPUT FROM DPPCO OR SPPFA.
C
C        N       INTEGER
C                THE ORDER OF THE MATRIX  A .
C
...
C
  ...
  180 CONTINUE
      ...
      DOUBLE PRECISION ABD(LDA,1)
C
C     SPBFA FACTORS A DOUBLE PRECISION SYMMETRIC POSITIVE DEFINITE
C     MATRIX STORED IN BAND FORM.
C
C     SPBFA IS USUALLY CALLED BY DPBCO, BUT IT CAN BE CALLED
C     DIRECTLY WITH A SAVING IN TIME IF  RCOND  IS NOT NEEDED.
C
C     ON ENTRY
C
...
C     BAND SYSTEM  A*X = B
C     USING THE FACTORS COMPUTED BY DPBCO OR SPBFA.
C
C     ON ENTRY
C
C        ABD     DOUBLE PRECISION(LDA, N)
C                THE OUTPUT FROM DPBCO OR SPBFA.
C
C        LDA     INTEGER
C                THE LEADING DIMENSION OF THE ARRAY  ABD .
C
...
C     INTERNAL VARIABLES
C
      DOUBLE PRECISION AK,AKM1,BK,BKM1,SDOT,DENOM,EK,T
      DOUBLE PRECISION ANORM,S,SASUM,YNORM
...
C
      ...
      DOUBLE PRECISION A(LDA,1)
C
C     SSIFA FACTORS A DOUBLE PRECISION SYMMETRIC MATRIX BY ELIMINATION
C     WITH SYMMETRIC PIVOTING.
C
C     TO SOLVE  A*X = B , FOLLOW SSIFA BY DSISL.
C     TO COMPUTE  INVERSE(A)*C , FOLLOW SSIFA BY DSISL.
C     TO COMPUTE  DETERMINANT(A) , FOLLOW SSIFA BY DSIDI.
C     TO COMPUTE  INERTIA(A) , FOLLOW SSIFA BY DSIDI.
C     TO COMPUTE  INVERSE(A) , FOLLOW SSIFA BY DSIDI.
C
C     ON ENTRY
C
C        A       DOUBLE PRECISION(LDA,N)
...
C     INTERNAL VARIABLES.
C
      DOUBLE PRECISION AKKP1,SDOT,TEMP
...
C
      ...
      DOUBLE PRECISION AP(1)
C
C     SSPFA FACTORS A DOUBLE PRECISION SYMMETRIC MATRIX STORED IN
C     PACKED FORM BY ELIMINATION WITH SYMMETRIC PIVOTING.
C
C     TO SOLVE  A*X = B , FOLLOW SSPFA BY DSPSL.
C     TO COMPUTE  INVERSE(A)*C , FOLLOW SSPFA BY DSPSL.
C     TO COMPUTE  DETERMINANT(A) , FOLLOW SSPFA BY DSPDI.
C     TO COMPUTE  INERTIA(A) , FOLLOW SSPFA BY DSPDI.
C     TO COMPUTE  INVERSE(A) , FOLLOW SSPFA BY DSPDI.
C
C     ON ENTRY
C
C        AP      DOUBLE PRECISION (N*(N+1)/2)
...
C     OF A DOUBLE PRECISION SYMMETRIC MATRIX USING THE FACTORS FROM
C     SSPFA, WHERE THE MATRIX IS STORED IN PACKED FORM.
C
C     ON ENTRY
C
C        AP      DOUBLE PRECISION (N*(N+1)/2)
...
C
      ...
      DOUBLE PRECISION TNORM,YNORM,S,SM,SASUM
...
C
      ...
      DO 10 J = 1, N
         ...
         I1 = 1
         IF (LOWER) I1 = J
         TNORM = DMAX1(TNORM,SASUM(L,T(I1,J),1))
   10 CONTINUE
C
...
C     INTERNAL VARIABLES
C
      INTEGER PU,PL,PLP1,J,JP,JT,K,KB,KM1,KP1,L,MAXL                  00
      DOUBLE PRECISION TEMP
...
C
      INTEGER I,II,J
      DOUBLE PRECISION A,ALPHA,AZETA,NORM,SNRM2
      DOUBLE PRECISION SDOT,T,ZETA,B,XX
C
...
C
      ...
      DOUBLE PRECISION MAXNRM,SNRM2,TT
      DOUBLE PRECISION SDOT,NRMXL,T
      LOGICAL NEGJ,SWAPJ
C
C
      PL = 1
...
C
     ...
     *        MM,MM1,MP1,NCT,NCTP1,NCU,NRT,NRTP1
      DOUBLE PRECISION SDOT,T
      DOUBLE PRECISION B,C,CS,EL,EMM1,F,G,SNRM2,SCALE,SHIFT,SL,SM,SN,
     *                 SMM1,T1,TEST,ZTEST
      LOGICAL WANTU,WANTV
C
C
...
C     FORTRAN ABS,DIMAG,MAX,DCMPLX,CONJG,DBLE
C
C     INTERNAL VARIABLES
C
...
C     FORTRAN ABS,DIMAG,DCMPLX,MOD,DBLE
C
C     INTERNAL VARIABLES
C
      DOUBLE COMPLEX T
...
C     LINPACK CGBFA
C     BLAS CAXPY,CDOTC,CSSCAL,SCASUM
C     FORTRAN ABS,DIMAG,MAX,DCMPLX,CONJG,MAX0,MIN0,DBLE
C
C     INTERNAL VARIABLES
C
      DOUBLE COMPLEX CDOTC,EK,T,WK,WKM
...
C
C     BLAS CAXPY,CSCAL,ICAMAX
C     FORTRAN ABS,DIMAG,MAX0,MIN0,DBLE
C
C     INTERNAL VARIABLES
C
      DOUBLE COMPLEX T
...
C
  ...
  100 CONTINUE
      ...
      INTEGER LDA,N,ML,MU,IPVT(1)
      DOUBLE COMPLEX ABD(LDA,1),DET(2)
C
...
C     SUBROUTINES AND FUNCTIONS
C
C     FORTRAN ABS,DIMAG,DCMPLX,DBLE
C
C     INTERNAL VARIABLES
C
      DOUBLE PRECISION TEN
...
C
      DO 30 J = 1, N
         Z(J) = DCMPLX(SCASUM(J,A(1,J),1),0.0D0)
         JM1 = J - 1
         IF (JM1 .LT. 1) GO TO 20
         DO 10 I = 1, JM1
            Z(I) = DCMPLX(DBLE(Z(I))+CABS1(A(I,J)),0.0D0)
   10    CONTINUE
   20    CONTINUE
   30 CONTINUE
      ANORM = 0.0D0
      DO 40 J = 1, N
         ANORM = MAX(ANORM,DBLE(Z(J)))
   40 CONTINUE
C
C     FACTOR
C
...
C
   ...
   50    CONTINUE
         DO 110 K = 1, N
            IF (CABS1(Z(K)) .NE. 0.0D0) EK = CSIGN1(EK,-Z(K))
            IF (CABS1(EK-Z(K)) .LE. DBLE(A(K,K))) GO TO 60
               S = DBLE(A(K,K))/CABS1(EK-Z(K))
               CALL CSSCAL(N,S,Z,1)
               EK = DCMPLX(S,0.0D0)*EK
   60       CONTINUE
            WK = EK - Z(K)
...
C
         DO 130 KB = 1, N
            K = N + 1 - KB
            IF (CABS1(Z(K)) .LE. DBLE(A(K,K))) GO TO 120
               S = DBLE(A(K,K))/CABS1(Z(K))
               CALL CSSCAL(N,S,Z,1)
  120       CONTINUE
            Z(K) = Z(K)/A(K,K)
            T = -Z(K)
...
C
         DO 150 K = 1, N
            ...
            IF (CABS1(Z(K)) .LE. DBLE(A(K,K))) GO TO 140
               S = DBLE(A(K,K))/CABS1(Z(K))
               CALL CSSCAL(N,S,Z,1)
               YNORM = S*YNORM
  140       CONTINUE
...
C
         DO 170 KB = 1, N
            K = N + 1 - KB
            IF (CABS1(Z(K)) .LE. DBLE(A(K,K))) GO TO 160
               S = DBLE(A(K,K))/CABS1(Z(K))
               CALL CSSCAL(N,S,Z,1)
               YNORM = S*YNORM
  160       CONTINUE
...
C
C     BLAS CDOTC
C     FORTRAN DIMAG,DCMPLX,CONJG,DBLE,SQRT
C
C     INTERNAL VARIABLES
C
      DOUBLE COMPLEX CDOTC,T
...
C
      J1 = 1
      DO 30 J = 1, N
         Z(J) = DCMPLX(SCASUM(J,AP(J1),1),0.0D0)
         IJ = J1
         J1 = J1 + J
         JM1 = J - 1
         IF (JM1 .LT. 1) GO TO 20
         DO 10 I = 1, JM1
            Z(I) = DCMPLX(DBLE(Z(I))+CABS1(AP(IJ)),0.0D0)
            IJ = IJ + 1
   10    CONTINUE
   20    CONTINUE
   30 CONTINUE
...
C
   ...
   50    CONTINUE
         ...
         DO 110 K = 1, N
            KK = KK + K
            IF (CABS1(Z(K)) .NE. 0.0D0) EK = CSIGN1(EK,-Z(K))
            IF (CABS1(EK-Z(K)) .LE. DBLE(AP(KK))) GO TO 60
               S = DBLE(AP(KK))/CABS1(EK-Z(K))
               CALL CSSCAL(N,S,Z,1)
               EK = DCMPLX(S,0.0D0)*EK
   60       CONTINUE
            WK = EK - Z(K)
...
C
         DO 130 KB = 1, N
            K = N + 1 - KB
            IF (CABS1(Z(K)) .LE. DBLE(AP(KK))) GO TO 120
               S = DBLE(AP(KK))/CABS1(Z(K))
               CALL CSSCAL(N,S,Z,1)
  120       CONTINUE
            Z(K) = Z(K)/AP(KK)
            KK = KK - K
...
C
         DO 150 K = 1, N
            ...
            KK = KK + K
            IF (CABS1(Z(K)) .LE. DBLE(AP(KK))) GO TO 140
               S = DBLE(AP(KK))/CABS1(Z(K))
               CALL CSSCAL(N,S,Z,1)
               YNORM = S*YNORM
  140       CONTINUE
...
C
         DO 170 KB = 1, N
            K = N + 1 - KB
            IF (CABS1(Z(K)) .LE. DBLE(AP(KK))) GO TO 160
               S = DBLE(AP(KK))/CABS1(Z(K))
               CALL CSSCAL(N,S,Z,1)
               YNORM = S*YNORM
  160       CONTINUE
...
C
      DO 30 J = 1, N
         L = MIN0(J,M+1)
         MU = MAX0(M+2-J,1)
         Z(J) = DCMPLX(SCASUM(L,ABD(MU,J),1),0.0D0)
         K = J - L
         IF (M .LT. MU) GO TO 20
         DO 10 I = MU, M
            K = K + 1
            Z(K) = DCMPLX(DBLE(Z(K))+CABS1(ABD(I,J)),0.0D0)
   10    CONTINUE
   20    CONTINUE
   30 CONTINUE
      ANORM = 0.0D0
...
C
   ...
   50    CONTINUE
         DO 110 K = 1, N
            IF (CABS1(Z(K)) .NE. 0.0D0) EK = CSIGN1(EK,-Z(K))
            IF (CABS1(EK-Z(K)) .LE. DBLE(ABD(M+1,K))) GO TO 60
               S = DBLE(ABD(M+1,K))/CABS1(EK-Z(K))
               CALL CSSCAL(N,S,Z,1)
               EK = DCMPLX(S,0.0D0)*EK
   60       CONTINUE
            WK = EK - Z(K)
...
C
         DO 130 KB = 1, N
            K = N + 1 - KB
            IF (CABS1(Z(K)) .LE. DBLE(ABD(M+1,K))) GO TO 120
               S = DBLE(ABD(M+1,K))/CABS1(Z(K))
               CALL CSSCAL(N,S,Z,1)
  120       CONTINUE
            Z(K) = Z(K)/ABD(M+1,K)
            LM = MIN0(K-1,M)
...
C
         DO 150 K = 1, N
            ...
            IF (CABS1(Z(K)) .LE. DBLE(ABD(M+1,K))) GO TO 140
               S = DBLE(ABD(M+1,K))/CABS1(Z(K))
               CALL CSSCAL(N,S,Z,1)
               YNORM = S*YNORM
  140       CONTINUE
...
C
         DO 170 KB = 1, N
            K = N + 1 - KB
            IF (CABS1(Z(K)) .LE. DBLE(ABD(M+1,K))) GO TO 160
               S = DBLE(ABD(M+1,K))/CABS1(Z(K))
               CALL CSSCAL(N,S,Z,1)
               YNORM = S*YNORM
  160       CONTINUE
...
C
C     BLAS CDOTC
C     FORTRAN DIMAG,DCMPLX,CONJG,MAX0,DBLE,SQRT
C
C     INTERNAL VARIABLES
C
      DOUBLE COMPLEX CDOTC,T
...
C     FORTRAN ABS,DIMAG,MAX,DCMPLX,IABS,DBLE
C
C     INTERNAL VARIABLES
C
      DOUBLE COMPLEX AK,AKM1,BK,BKM1,CDOTU,DENOM,EK,T
      DOUBLE PRECISION ANORM,S,SCASUM,YNORM
      INTEGER I,INFO,J,JM1,K,KP,KPS,KS
C
      DOUBLE COMPLEX ZDUM,ZDUM2,CSIGN1
      DOUBLE PRECISION CABS1
...
C     FORTRAN ABS,DIMAG,MAX,DBLE,SQRT
C
C     INTERNAL VARIABLES
C
...
C     INTERNAL VARIABLES.
C
      DOUBLE COMPLEX AK,AKM1,BK,BKM1,CDOTU,DENOM,TEMP
      INTEGER K,KP
C
C     LOOP BACKWARD APPLYING THE TRANSFORMATIONS AND
C     D INVERSE TO B.
...
C     FORTRAN ABS,DCMPLX,IABS,MOD,DBLE
C
C     INTERNAL VARIABLES.
C
      DOUBLE COMPLEX AK,AKP1,AKKP1,CDOTU,D,T,TEMP
      DOUBLE PRECISION TEN
      INTEGER J,JB,K,KM1,KS,KSTEP
      LOGICAL NOINV,NODET
C
...
C
  ...
  160 CONTINUE
      ...
      INTEGER N,JOB
      DOUBLE COMPLEX AP(1),WORK(1),DET(2)
      INTEGER KPVT(1)
C
...
C     INTERNAL VARIABLES.
C
      DOUBLE COMPLEX AK,AKKP1,AKP1,CDOTU,D,T,TEMP
      DOUBLE PRECISION TEN
      INTEGER IJ,IK,IKP1,IKS,J,JB,JK,JKP1
      INTEGER K,KK,KKP1,KM1,KS,KSJ,KSKP1,KSTEP
      LOGICAL NOINV,NODET
...
C     LINPACK CHIFA
C     BLAS CAXPY,CDOTC,CSSCAL,SCASUM
C     FORTRAN ABS,DIMAG,MAX,DCMPLX,CONJG,IABS,DBLE
C
C     INTERNAL VARIABLES
C
      DOUBLE COMPLEX AK,AKM1,BK,BKM1,CDOTC,DENOM,EK,T
      DOUBLE PRECISION ANORM,S,SCASUM,YNORM
      INTEGER I,INFO,J,JM1,K,KP,KPS,KS
C
      DOUBLE COMPLEX ZDUM,ZDUM2,CSIGN1
...
C
C     BLAS CAXPY,CSWAP,ICAMAX
C     FORTRAN ABS,DIMAG,MAX,DCMPLX,CONJG,DBLE,SQRT
C
C     INTERNAL VARIABLES
C
      DOUBLE COMPLEX AK,AKM1,BK,BKM1,DENOM,MULK,MULKM1,T
...
C     INTERNAL VARIABLES.
C
      DOUBLE COMPLEX AK,AKM1,BK,BKM1,CDOTC,DENOM,TEMP
      INTEGER K,KP
C
C     LOOP BACKWARD APPLYING THE TRANSFORMATIONS AND
C     D INVERSE TO B.
...
C     FORTRAN ABS,CABS,DCMPLX,CONJG,IABS,MOD,DBLE
C
C     INTERNAL VARIABLES.
C
      DOUBLE COMPLEX AKKP1,CDOTC,TEMP
      DOUBLE PRECISION TEN,D,T,AK,AKP1
      INTEGER J,JB,K,KM1,KS,KSTEP
      LOGICAL NOINV,NODET,NOERT
C
...
C              1 BY 1
C
               A(K,K) = DCMPLX(1.0D0/DBLE(A(K,K)),0.0D0)
               IF (KM1 .LT. 1) GO TO 170
                  CALL CCOPY(KM1,A(1,K),1,WORK,1)
                  DO 160 J = 1, KM1
  ...
  160             CONTINUE
                  A(K,K) = A(K,K)
     *                     + DCMPLX(DBLE(CDOTC(KM1,WORK,1,A(1,K),1)),
     *                             0.0D0)
  170          CONTINUE
               KSTEP = 1
            GO TO 220
...
C
            ...
            DO 130 JJ = 1, KM1
               ...
               CALL CAXPY(J,T,AP(IK+1),1,AP(IJ+1),1)
               IJJ = IJ + J
               AP(IJJ) = DCMPLX(DBLE(AP(IJJ)),0.0D0)
               AP(JK) = MULK
               IJ = IJ - (J - 1)
  130       CONTINUE
C
...
C              1 BY 1
C
               AP(KK) = DCMPLX(1.0D0/DBLE(AP(KK)),0.0D0)
               IF (KM1 .LT. 1) GO TO 170
                  CALL CCOPY(KM1,AP(IK+1),1,WORK,1)
                  IJ = 0
                  DO 160 J = 1, KM1
  ...
  160             CONTINUE
                  AP(KK) = AP(KK)
     *                     + DCMPLX(DBLE(CDOTC(KM1,WORK,1,AP(IK+1),1)),
     *                             0.0D0)
  170          CONTINUE
               KSTEP = 1
            GO TO 220
...
C     INTERNAL VARIABLES
C
      DOUBLE COMPLEX W,WK,WKM,EK
      DOUBLE PRECISION TNORM,YNORM,S,SM,SCASUM
      INTEGER I1,J,J1,J2,K,KK,L
      LOGICAL LOWER
      DOUBLE COMPLEX ZDUM,ZDUM1,ZDUM2,CSIGN1
      DOUBLE PRECISION CABS1
...
C
C     BLAS CAXPY,CDOTC
C     FORTRAN ABS,DIMAG,CONJG,MOD,DBLE
C
C     INTERNAL VARIABLES
C
      DOUBLE COMPLEX CDOTC,TEMP
      INTEGER CASE,J,JJ
C
      DOUBLE COMPLEX ZDUM
      DOUBLE PRECISION CABS1
...
C     ............EXIT
  ...
  180 CONTINUE
      ...
      INTEGER N,INFO
      DOUBLE COMPLEX C(1),D(1),E(1),B(1)
C
...
C
         IF (K .LT. PL .OR. K .GE. PU) GO TO 190
            DO 180 L = KP1, PU
               IF (DBLE(A(L,L)) .LE. MAXDIA) GO TO 170
                  MAXDIA = DBLE(A(L,L))
                  MAXL = L
  170          CONTINUE
  180       CONTINUE
...
C        REDUCTION STEP. PIVOTING IS CONTAINED ACROSS THE ROWS.
C
         WORK(K) = DCMPLX(SQRT(DBLE(A(K,K))),0.0D0)
         A(K,K) = WORK(K)
         IF (P .LT. KP1) GO TO 260
         DO 250 J = KP1, P
            IF (K .EQ. MAXL) GO TO 240
  ...
  280 CONTINUE
      ...
      INTEGER LDR,P,LDZ,NZ
      DOUBLE PRECISION RHO(1),C(1)
      DOUBLE COMPLEX R(LDR,1),X(1),Z(LDZ,1),Y(1),S(1)
C
...
C
C     FORTRAN DIMAG,CABS,CONJG
C     BLAS CDOTC, SCNRM2
C
      INTEGER I,II,J
      DOUBLE PRECISION A,ALPHA,AZETA,NORM,SCNRM2
      DOUBLE COMPLEX CDOTC,T,ZETA,B,XX
C
...
C
         DO 40 II = 1, P
            ...
            A = ALPHA/SCALE
            B = S(I)/SCALE
            NORM = SQRT(A**2+DBLE(B)**2+DIMAG(B)**2)
            C(I) = A/NORM
            S(I) = CONJG(B)/NORM
            ALPHA = SCALE*NORM
   40    CONTINUE
...
C
  ...
  120 CONTINUE
      ...
      INTEGER LDR,P,K,L,LDZ,NZ,JOB
      DOUBLE COMPLEX R(LDR,1),Z(LDZ,1),S(1)
      DOUBLE PRECISION C(1)
C
...
C
  ...
  260 CONTINUE
      ...
      INTEGER LDX,N,P,JOB
      INTEGER JPVT(1)
      DOUBLE COMPLEX X(LDX,1),QRAUX(1),WORK(1)
C
...
C     FORTRAN ABS,DIMAG,MAX,CABS,DCMPLX,CSQRT,MIN0,DBLE
C
C     INTERNAL VARIABLES
C
      INTEGER J,JP,L,LP1,LUP,MAXJ,PL,PU
      DOUBLE PRECISION MAXNRM,SCNRM2,TT
      DOUBLE COMPLEX CDOTC,NRMXL,T
      LOGICAL NEGJ,SWAPJ
C
      DOUBLE COMPLEX CSIGN,ZDUM,ZDUM1,ZDUM2
      DOUBLE PRECISION CABS1
...
C
            ...
            MAXJ = L
            DO 100 J = L, PU
               IF (DBLE(QRAUX(J)) .LE. MAXNRM) GO TO 90
                  MAXNRM = DBLE(QRAUX(J))
                  MAXJ = J
   90          CONTINUE
  100       CONTINUE
...
C
     ...
     *            NRMXL = CSIGN(NRMXL,X(L,L))
               CALL CSCAL(N-L+1,(1.0D0,0.0D0)/NRMXL,X(L,L),1)
...
C
               ...
               DO 160 J = LP1, P
                  ...
                  IF (CABS1(QRAUX(J)) .EQ. 0.0D0) GO TO 150
                     TT = 1.0D0 - (ABS(X(L,J))/DBLE(QRAUX(J)))**2
                     TT = MAX(TT,0.0D0)
                     T = DCMPLX(TT,0.0D0)
                     TT = 1.0D0
     *                    + 0.05E0*TT*(DBLE(QRAUX(J))/DBLE(WORK(J)))**2
                     IF (TT .EQ. 1.0D0) GO TO 130
...
C
  ...
  200 CONTINUE
      ...
      INTEGER LDX,N,K,JOB,INFO
      DOUBLE COMPLEX X(LDX,1),QRAUX(1),Y(1),QY(1),QTY(1),B(1),RSD(1),
     *     XB(1)
C
...
C     FORTRAN ABS,DIMAG,MIN0,MOD,DBLE
C
C     INTERNAL VARIABLES
C
      INTEGER I,J,JJ,JU,KP1
      DOUBLE COMPLEX CDOTC,T,TEMP
      LOGICAL CB,CQY,CQTY,CR,CXB
C
      DOUBLE COMPLEX ZDUM
      DOUBLE PRECISION CABS1
...
C
  ...
  250 CONTINUE
      ...
      INTEGER LDX,N,P,LDU,LDV,JOB,INFO
      DOUBLE COMPLEX X(LDX,1),S(1),E(1),U(LDU,1),V(LDV,1),WORK(1)
C
C
...
C     FORTRAN ABS,DIMAG,MAX,CABS,DCMPLX
C     FORTRAN CONJG,MAX0,MIN0,MOD,DBLE,SQRT
C
C     INTERNAL VARIABLES
C
      INTEGER I,ITER,J,JOBU,K,KASE,KK,L,LL,LLS,LM1,LP1,LS,LU,M,MAXIT,
     *        MM,MM1,MP1,NCT,NCTP1,NCU,NRT,NRTP1
      DOUBLE COMPLEX CDOTC,T,R
      DOUBLE PRECISION B,C,CS,EL,EMM1,F,G,SCNRM2,SCALE,SHIFT,SL,SM,
...
C           PLACE THE L-TH DIAGONAL IN S(L).
C
            S(L) = DCMPLX(SCNRM2(N-L+1,X(L,L),1),0.0D0)
...
C           L-TH SUPER-DIAGONAL IN E(L).
C
            E(L) = DCMPLX(SCNRM2(P-L,E(LP1),1),0.0D0)
...
C
  ...
  210          CONTINUE
  220          CONTINUE
               CALL CSCAL(N-L+1,(-1.0D0,0.0D0),U(L,L),1)
...
C
     ...
     *                    ABS(S(L)),ABS(E(L)))
            SM = DBLE(S(M))/SCALE
            SMM1 = DBLE(S(M-1))/SCALE
            EMM1 = DBLE(E(M-1))/SCALE
            SL = DBLE(S(L))/SCALE
            EL = DBLE(E(L))/SCALE
            B = ((SMM1 + SM)*(SMM1 - SM) + EMM1**2)/2.0D0
            C = (SM*EMM1)**2
...
C           MAKE THE SINGULAR VALUE  POSITIVE
C
            IF (DBLE(S(L)) .GE. 0.0D0) GO TO 620
               S(L) = -S(L)
               IF (WANTV) CALL CSCAL(P,(-1.0D0,0.0D0),V(1,L),1)
  620       CONTINUE
C
C           ORDER THE SINGULAR VALUE.
C
...
C           ...EXIT
               IF (DBLE(S(L)) .GE. DBLE(S(L+1))) GO TO 640
               T = S(L)
               S(L) = S(L+1)
               S(L+1) = T
               IF (WANTV .AND. L .LT. P)
