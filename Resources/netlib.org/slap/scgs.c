/* scgs.f -- translated by f2c (version 20100827).
...
/*      REAL    Q(N), U(N), V1(N), V2(N), RWORK(USER DEFINABLE) */
...
/*       routines SDBCG and SSLUCS are examples of this procedure. */
...
/*                 Phone 015-784568. */
/* ***ROUTINES CALLED  MATVEC, MSOLVE, ISSCGS, SDOT, R1MACH */
/* ***END PROLOGUE  SCGS */
...
/* DECK SSDCGS */
/* Subroutine */ int ssdcgs_(integer *n, real *b, real *x, integer *nelt, 
	...
	leniw)
{
    ...
    static integer locr0, locv1, locv2, lociw;
...
/* ***CATEGORY NO.  D2A4 */
/* ***KEYWORDS  LIBRARY=SLATEC(SLAP), */
/*             TYPE=SINGLE PRECISION(SSDCGS-S), */
/*             Non-Symmetric Linear system, Sparse, */
...
/*                 Informatics, Julianalaan 132, 2628 BL Delft, */
/*                 Phone 015-784568. */
/* ***ROUTINES CALLED  SS2Y, SCHKW, SSDS, SCGS */
/* ***END PROLOGUE  SSDCGS */
...
/*         Check the workspace allocations. */
    schkw_("SSDCGS", &lociw, leniw, &locw, lenw, ierr, iter, err, (ftnlen)6);
    if (*ierr != 0) {
	return 0;
    }
...
/*         BiConjugate Gradient Squared algorithm. */
    scgs_(n, &b[1], &x[1], nelt, &ia[1], &ja[1], &a[1], isym, (S_fp)ssmv_, (
	    S_fp)ssdi_, itol, tol, itmax, iter, err, ierr, iunit, &rwork[locr]
	    , &rwork[locr0], &rwork[locp], &rwork[locq], &rwork[locu], &rwork[
	    locv1], &rwork[locv2], &rwork[1], &iwork[1]);
    return 0;
...
/* ***CATEGORY NO.  D2A4 */
/* ***KEYWORDS  LIBRARY=SLATEC(SLAP), */
/*             TYPE=SINGLE PRECISION(SSLUCS-S), */
/*             Non-Symmetric Linear system, Sparse, */
...
/*                 Informatics, Julianalaan 132, 2628 BL Delft, */
/*                 Phone 015-784568. */
/* ***ROUTINES CALLED  SS2Y, SCHKW, SSILUS, SCGS, SSMV, SSLUI */
/* ***END PROLOGUE  SSLUCS */
...
/* DECK ISSCGS */
integer isscgs_(integer *n, real *b, real *x, integer *nelt, integer *ia, 
	integer *ja, real *a, integer *isym, S_fp matvec, S_fp msolve, 
	integer *itol, real *tol, integer *itmax, integer *iter, real *err, 
	integer *ierr, integer *iunit, real *r__, real *r0, real *p, real *q, 
	real *u, real *v1, real *v2, real *rwork, integer *iwork, real *ak, 
	real *bk, real *bnrm, real *solnrm)
{
    /* Format strings */
    static char fmt_1000[] = "(\002 Preconditioned BiConjugate Gradient Squa"
	    "red for \002,\002N, ITOL = \002,i5,i5,/\002 ITER\002,\002   Erro"
	    "r Estimate\002,\002            Alpha\002,\002             Bet"
	    "a\002)";
    ...
    static cilist io___49 = { 0, 0, 0, fmt_1010, 0 };


/* ***BEGIN PROLOGUE  ISSCGS */
/* ***REFER TO  SCGS, SSDCGS, SSLUCS */
/* ***DATE WRITTEN   871119   (YYMMDD) */
/* ***REVISION DATE  881213   (YYMMDD) */
/* ***CATEGORY NO.  D2A4 */
/* ***KEYWORDS  LIBRARY=SLATEC(SLAP), */
/*             TYPE=SINGLE PRECISION(ISSCGS-S), */
/*             Non-Symmetric Linear system, Sparse, */
...
/*     REAL     B(N), X(N), A(N), TOL, ERR, R(N), R0(N), P(N), Q(N) */
/*     REAL     U(N), V1(N), V2(N) */
...
/*         the SLAP routine SCGS for more late breaking details... */
/* ISYM   :IN       Integer. */
/*         Flag to indicate symmetric storage format. */
/*         If ISYM=0, all nonzero entries of the matrix are stored. */
/*         If ISYM=1, the matrix is symmetric, and only the upper */
...
/* *Precision:           Single Precision */
/* ***REFERENCES  (NONE) */
/* ***ROUTINES CALLED  MATVEC, MSOLVE, SNRM2 */
...
/*         err = ||Residual||/||RightHandSide|| (2-Norms). */
	...
	(*matvec)(n, &x[1], &v2[1], nelt, &ia[1], &ja[1], &a[1], isym);
	i__1 = *n;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    v2[i__] -= b[i__];
/* L5: */
	}
	*err = snrm2_(n, &v2[1], &c__1) / *bnrm;
    } else if (*itol == 2) {
/*                  -1              -1 */
/*         err = ||M  Residual||/||M  RightHandSide|| (2-Norms). */
	if (*iter == 0) {
	    (*msolve)(n, &b[1], &v2[1], nelt, &ia[1], &ja[1], &a[1], isym, &
		    rwork[1], &iwork[1]);
	    *bnrm = snrm2_(n, &v2[1], &c__1);
	}
...
/*         err = ||x-TrueSolution||/||TrueSolution|| (2-Norms). */
	...
	i__1 = *n;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    v2[i__] = x[i__] - solblk_1.soln[i__ - 1];
/* L10: */
	}
	*err = snrm2_(n, &v2[1], &c__1) / *solnrm;
    } else {
