...
C
C     CALL SIR(N, B, X, NELT, IA, JA, A, ISYM, MATVEC, MSLOVE, ITOL, 
...
C       routine).
C       If   M  =   DIAG(A), the   diagonal   of A,  then iterative
C       refinement is known  as Jacobi's method.   The SLAP  routine
C       SSJAC implements this iterative strategy.
...
C***BEGIN PROLOGUE  SSJAC
C***DATE WRITTEN   871119   (YYMMDD)
C***REVISION DATE  881213   (YYMMDD)
C***CATEGORY NO.  D2A4, D2B4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(SSJAC-S),
C             Linear system, Sparse, Iterative Precondition
C***AUTHOR  Greenbaum, Anne, Courant Institute
C           Seager, Mark K., (LLNL)
C             Lawrence Livermore National Laboratory
...
C
C     CALL SSJAC(N, B, X, NELT, IA, JA, A, ISYM, ITOL, TOL,
...
C LENW   :IN       Integer.
C         Length of the real workspace, RWORK.  LENW >= 4*N.
...
C
C***REFERENCES  (NONE)
C***ROUTINES CALLED  SS2Y, SSCHKW, SSDS, SIR, SSMV, SSDI
C***END PROLOGUE  SSJAC
      INTEGER N, NELT, IA(NELT), JA(NELT), ISYM, ITOL, ITMAX
      INTEGER ITER, IUNIT, LENW, IWORK(LENIW), LENIW
      REAL    B(N), X(N), A(NELT), RWORK(LENW)
      EXTERNAL SSMV, SSDI
...
C------------- LAST LINE OF SSJAC FOLLOWS -----------------------------
      END
*DECK SSGS
      SUBROUTINE SSGS(N, B, X, NELT, IA, JA, A, ISYM, ITOL, TOL,
     $     ITMAX, ITER, ERR, IERR, IUNIT, RWORK, LENW, IWORK, LENIW )
...
C***CATEGORY NO.  D2A4, D2B4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(SSGS-S),
C             Linear system, Sparse, Iterative Precondition
C***AUTHOR  Greenbaum, Anne, Courant Institute
C           Seager, Mark K., (LLNL)
C             Lawrence Livermore National Laboratory
...
C     REAL B(N), X(N), A(NELT), TOL, ERR, RWORK(NEL+3*N)
C
...
C         diagonal).  
C LENW   :IN       Integer.
C         Length of the real workspace, RWORK.  LENW >= NEL+3*N.
C IWORK  :WORK     Integer IWORK(LENIW).
...
C       SSJAC, SIR
C***REFERENCES  (NONE)
C***ROUTINES CALLED  SS2Y, SCHKW, SS2LT, SSIR, SSMV, SSLI
C***END PROLOGUE  SSGS
      INTEGER N, NELT, IA(NELT), JA(NELT), ISYM, ITOL, ITMAX
      INTEGER ITER, IUNIT, IWORK(10)
      REAL    B(N), X(N), A(N), TOL, ERR, RWORK(1)
      EXTERNAL SSMV, SSLI
      PARAMETER(LOCRB=1,LOCIB=11)
C
...
C
      ...
      IWORK(10) = LOCW
C
      CALL SS2LT( N, NELT, IA, JA, A, ISYM, NEL, IWORK(LOCIEL),
...
C------------- LAST LINE OF SSGS FOLLOWS ------------------------------
      END
*DECK SSILUR
      SUBROUTINE SSILUR(N, B, X, NELT, IA, JA, A, ISYM, ITOL, TOL,
     $     ITMAX, ITER, ERR, IERR, IUNIT, RWORK, LENW, IWORK, LENIW )
C***BEGIN PROLOGUE  SSILUR
C***DATE WRITTEN   871119   (YYMMDD)
C***REVISION DATE  881213   (YYMMDD)
C***CATEGORY NO.  D2A4, D2B4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(SSILUR-S),
C             Linear system, Sparse, Iterative Precondition
C***AUTHOR  Greenbaum, Anne, Courant Institute
C           Seager, Mark K., (LLNL)
C             Lawrence Livermore National Laboratory
...
C     REAL B(N), X(N), A(NELT), TOL, ERR, RWORK(NEL+NU+4*N)
C
C     CALL SSILUR(N, B, X, NELT, IA, JA, A, ISYM, ITOL, TOL,
C    $     ITMAX, ITER, ERR, IERR, IUNIT, RWORK, LENW, IWORK, LENIW)
C
C *Arguments:
C N      :IN       Integer
...
C LENW   :IN       Integer.
C         Length of the real workspace, RWORK.  LENW >= NEL+NU+4*N.
C IWORK  :WORK     Integer IWORK(LENIW).
C         Integer array used for workspace.  NEL is the number of non-
C         zeros in the lower triangle of the matrix (including the
C         diagonal).  NU is the number of nonzeros in the upper
...
C       SSJAC, SSGS, SIR
C***REFERENCES  (NONE)
C***ROUTINES CALLED  SS2Y, SCHKW, SSILUS, SIR, SSMV, SSLUI
C***END PROLOGUE  SSILUR
...
C***FIRST EXECUTABLE STATEMENT  SSILUR
      IERR = 0
      IF( N.LT.1 .OR. NELT.LT.1 ) THEN
         IERR = 3
         RETURN
...
C         Do the Preconditioned Iterative Refinement iteration.
      CALL SIR(N, B, X, NELT, IA, JA, A, ISYM, SSMV, SSLUI, 
     $     ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT, RWORK(LOCR),
     $     RWORK(LOCZ), RWORK(LOCDZ), RWORK, IWORK)
      RETURN
C------------- LAST LINE OF SSILUR FOLLOWS ----------------------------
      END
*DECK ISSIR
      FUNCTION ISSIR(N, B, X, NELT, IA, JA, A, ISYM, MSOLVE, ITOL, TOL,
     $     ITMAX, ITER, ERR, IERR, IUNIT, R, Z, DZ, RWORK, IWORK,
     $     BNRM, SOLNRM)
C***BEGIN PROLOGUE  ISSIR
C***REFER TO  SIR, SSJAC, SSGS
C***DATE WRITTEN   871119   (YYMMDD)
C***REVISION DATE  880320   (YYMMDD)
C***CATEGORY NO.  D2A4, D2B4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(ISSIR-S),
...
C***END PROLOGUE  ISSIR
      INTEGER N, NELT, IA(NELT), JA(NELT), ISYM, ITOL, ITMAX, ITER
      INTEGER IUNIT, IWORK(*)
      REAL    B(N), X(N), A(NELT), R(N), Z(N), DZ(N), RWORK(*)
      EXTERNAL MSOLVE
