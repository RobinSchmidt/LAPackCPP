*DECK SCGS
      SUBROUTINE SCGS(N, B, X, NELT, IA, JA, A, ISYM, MATVEC,
...
C             TYPE=SINGLE PRECISION(SCGS-S),
...
C      REAL    Q(N), U(N), V1(N), V2(N), RWORK(USER DEFINABLE)
...
C       routines SDBCG and SSLUCS are examples of this procedure.
C       
...
C                 Phone 015-784568.
C***ROUTINES CALLED  MATVEC, MSOLVE, ISSCGS, SDOT, R1MACH
C***END PROLOGUE  SCGS
      INTEGER N, NELT, IA(NELT), JA(NELT), ISYM, ITOL, ITMAX
      INTEGER ITER, IERR, IUNIT, IWORK(*)
      REAL    B(N), X(N), A(NELT), TOL, ERR, R(N), R0(N), P(N)
      REAL    Q(N), U(N), V1(N), V2(N), RWORK(*)
...
C***CATEGORY NO.  D2A4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(SSDCGS-S),
C             Non-Symmetric Linear system, Sparse, 
...
C                 Informatics, Julianalaan 132, 2628 BL Delft, 
C                 Phone 015-784568.
C***ROUTINES CALLED  SS2Y, SCHKW, SSDS, SCGS
...
C***CATEGORY NO.  D2A4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(SSLUCS-S),
C             Non-Symmetric Linear system, Sparse, 
...
C                 Informatics, Julianalaan 132, 2628 BL Delft, 
C                 Phone 015-784568.
C***ROUTINES CALLED  SS2Y, SCHKW, SSILUS, SCGS, SSMV, SSLUI
...
C------------- LAST LINE OF SSLUCS FOLLOWS ----------------------------
      END
*DECK ISSCGS
      FUNCTION ISSCGS(N, B, X, NELT, IA, JA, A, ISYM, MATVEC, MSOLVE, 
     $     ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT, R, R0, P, Q, U, 
     $     V1, V2, RWORK, IWORK, AK, BK, BNRM, SOLNRM)
C***BEGIN PROLOGUE  ISSCGS
C***REFER TO  SCGS, SSDCGS, SSLUCS
C***DATE WRITTEN   871119   (YYMMDD)
C***REVISION DATE  881213   (YYMMDD)
C***CATEGORY NO.  D2A4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(ISSCGS-S),
C             Non-Symmetric Linear system, Sparse,
...
C     REAL     B(N), X(N), A(N), TOL, ERR, R(N), R0(N), P(N), Q(N)
C     REAL     U(N), V1(N), V2(N)
...
C         the SLAP routine SCGS for more late breaking details...
C ISYM   :IN       Integer.
C         Flag to indicate symmetric storage format.
C         If ISYM=0, all nonzero entries of the matrix are stored.
C         If ISYM=1, the matrix is symmetric, and only the upper
