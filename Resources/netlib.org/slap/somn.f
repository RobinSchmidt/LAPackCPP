...
C             TYPE=SINGLE PRECISION(SOMN-S),
...
C     REAL     B(N), X(N), A(NELT), TOL, ERR, R(N), Z(N), P(N,0:NSAVE)
C     REAL     AP(N,0:NSAVE), EMAP(N,0:NSAVE), DZ(N)
C     REAL     CSAV(NSAVE), RWORK(USER DEFIED)
...
C
C***REFERENCES  (NONE)
C***ROUTINES CALLED  MATVEC, MSOLVE, ISSOMN, 
...
C***END PROLOGUE  SOMN
      ...
      REAL     P(N,0:NSAVE), AP(N,0:NSAVE), EMAP(N,0:NSAVE), DZ(N)
      REAL     CSAV(NSAVE), RWORK(*)
...
C------------- LAST LINE OF SOMN FOLLOWS ----------------------------
      END
*DECK SSDOMN
      SUBROUTINE SSDOMN(N, B, X, NELT, IA, JA, A, ISYM, NSAVE,
     $     ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT,
...
C***CATEGORY NO.  D2A4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(SSDOMN-S),
...
C     REAL B(N), X(N), A(NELT), TOL, ERR, RWORK(7*N+3*N*NSAVE+NSAVE)
C
...
C LENIW  :IN       Integer.
C         Length of the real workspace, RWORK.  LENW >= 10.
C         
C *Description:
...
C         SOMN, SSLUOM
C***REFERENCES  (NONE)
C***ROUTINES CALLED  SS2Y, SCHKW, SSDS, SOMN, SSMV, SSDI
...
C------------- LAST LINE OF SSDOMN FOLLOWS ----------------------------
      END
*DECK SSLUOM
      SUBROUTINE SSLUOM(N, B, X, NELT, IA, JA, A, ISYM, NSAVE,
     $     ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT,
     $     RWORK, LENW, IWORK, LENIW )
...
C***CATEGORY NO.  D2A4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(SSLUOM-S),
C             Non-Symmetric Linear system, Sparse, 
...
C     REAL RWORK(NEL+NU+7*N+3*N*NSAVE+NSAVE)
C
...
C LENIW  :IN       Integer.
C         Length of the real workspace, RWORK.  LENW > NL+NU+4*N+12.
C
C *Description:
C       This routine is  simply a driver  for  the SOMN routine.  It
...
C         SOMN, SSDOMN
C***REFERENCES  (NONE)
C***ROUTINES CALLED  SS2Y, SCHKW, SSILUS, SOMN, SSMV, SSLUI
C***END PROLOGUE  SSLUOM
      INTEGER N, NELT, IA(NELT), JA(NELT), ISYM, NSAVE, ITOL, ITMAX
      INTEGER ITER, IERR, IUNIT, LENW, IWORK(LENIW), LENIW
      REAL    B(N), X(N), A(N), RWORK(LENW)
      EXTERNAL SSMV, SSLUI
      PARAMETER (LOCRB=1, LOCIB=11)
C
C         Change the SLAP input matrix IA, JA, A to SLAP-Column format.
...
C         Perform the incomplete LU preconditioned OrthoMin algorithm.
     ...
     $     RWORK, IWORK )
      ...
      END
*DECK ISSOMN
      FUNCTION ISSOMN(N, B, X, NELT, IA, JA, A, ISYM, MSOLVE, NSAVE,
     $     ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT,
     $     R, Z, P, AP, EMAP, DZ, CSAV,
     $     RWORK, IWORK, AK, BNRM, SOLNRM)
C***BEGIN PROLOGUE  ISSOMN
C***REFER TO  SOMN, SSDOMN, SSLUOM
C***DATE WRITTEN   871119   (YYMMDD)
C***REVISION DATE  881213   (YYMMDD)
C***CATEGORY NO.  D2A4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(ISSOMN-S),
C             Non-Symmetric Linear system, Sparse, 
...
C     REAL     B(N), X(N), A(NELT), TOL, ERR, R(N), Z(N), P(N,0:NSAVE)
C     REAL     AP(N,0:NSAVE), EMAP(N,0:NSAVE), DZ(N)
C     REAL     CSAV(NSAVE), RWORK(USER DEFIED), AK, BNRM, SOLNRM
C     EXTERNAL MSOLVE
C
...
C         These arrays should hold the matrix A in either the SLAP
C         Triad format or the SLAP Column format.  See "LONG
C         DESCRIPTION" in the SSDOMN or SSLUOM.
C ISYM   :IN       Integer.
C         Flag to indicate symmetric storage format.
C         If ISYM=0, all nonzero entries of the matrix are stored.
C         If ISYM=1, the matrix is symmetric, and only the upper
