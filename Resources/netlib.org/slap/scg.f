...
C     REAL     B(N), X(N), A(NELT), TOL, ERR, R(N), Z(N), P(N), DZ(N)
C     REAL     RWORK(USER DEFINABLE)
C     EXTERNAL MATVEC, MSOLVE
C
C     CALL SCG(N, B, X, NELT, IA, JA, A, ISYM, MATVEC, MSLOVE, 
...
C***ROUTINES CALLED  MATVEC, MSOLVE, ISSCG, SCOPY, SDOT, SAXPY, R1MACH
C***END PROLOGUE  SCG
...
C***CATEGORY NO.  D2B4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(SSDCG-S),
C             Symmetric Linear system, Sparse, Iterative Precondition
C***AUTHOR  Greenbaum, Anne, Courant Institute
C           Seager, Mark K., (LLNL)
C             Lawrence Livermore National Laboratory
...
C                 Matrix Computations (Bunch \& Rose, Eds.), Academic 
C                 Press, New York (1979).
C***ROUTINES CALLED  SS2Y, SCHKW, SSDS, SCG
...
*DECK SSICCG
     ...
     $     ITMAX, ITER, ERR, IERR, IUNIT, RWORK, LENW, IWORK, LENIW )
C***BEGIN PROLOGUE  SSICCG
C***DATE WRITTEN   871119   (YYMMDD)
C***REVISION DATE  881213   (YYMMDD)
C***CATEGORY NO.  D2B4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(SSICCG-S),
...
C     REAL B(N), X(N), A(NELT), TOL, ERR, RWORK(NEL+5*N)
C
C     CALL SSICCG(N, B, X, NELT, IA, JA, A, ISYM, ITOL, TOL,
C    $     ITMAX, ITER, ERR, IERR, IUNIT, RWORK, LENW, IWORK, LENIW )
C
C *Arguments:
C N      :IN       Integer.
...
C         diagonal)
C LENW   :IN       Integer.
C         Length of the real workspace, RWORK.  LENW >= NEL+5*N.
C IWORK  :WORK     Integer IWORK(LENIW).
...
C       factorization of the matrix A.  See  SSICS for details about
...
C       possible.  The   MSOLVE operation (handled  in  SSLLTI) does
...
C                 Matrix Computations (Bunch \& Rose, Eds.), Academic 
C                 Press, New York (1979).
C***ROUTINES CALLED  SS2Y, SCHKW, SSICS, XERRWV, SCG
C***END PROLOGUE  SSICCG
      INTEGER N, NELT, IA(NELT), JA(NELT), ISYM, ITOL
      INTEGER ITMAX, ITER, IUNIT, LENW, IWORK(LENIW), LENIW
      REAL    B(N), X(N), A(NELT), TOL, ERR, RWORK(LENW)
      EXTERNAL SSMV, SSLLTI
      PARAMETER (LOCRB=1, LOCIB=11)
C         
...
C
      CALL SSICS(N, NELT, IA, JA, A, ISYM, NEL, IWORK(LOCIEL),
     $     IWORK(LOCJEL), RWORK(LOCEL), RWORK(LOCDIN),
     $     RWORK(LOCR), IERR )
      IF( IERR.NE.0 ) THEN
         CALL XERRWV('SSICCG: Warning...IC factorization broke down '//
...
C------------- LAST LINE OF SSICCG FOLLOWS ----------------------------
      END
*DECK ISSCG
      FUNCTION ISSCG(N, B, X, NELT, IA, JA, A, ISYM, MSOLVE, ITOL,
     $     TOL, ITMAX, ITER, ERR, IERR, IUNIT, R, Z, P, DZ, 
     $     RWORK, IWORK, AK, BK, BNRM, SOLNRM)
C***BEGIN PROLOGUE  ISSCG
C***REFER TO  SCG, SSDCG, SSICCG
C***DATE WRITTEN   871119   (YYMMDD)
C***REVISION DATE  881213   (YYMMDD)
C***CATEGORY NO.  D2B4
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(ISSCG-S),
...
C     REAL    B(N), X(N), A(N), TOL, ERR, R(N), Z(N), P(N), DZ(N)
